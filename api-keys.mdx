...
Every request authenticates with an API key, and every key spends your one wallet.
Manage keys at [dashboard > API keys](https://getanyapi.com/dashboard/keys), or call the
endpoints below with any key or session.
...
- `DELETE /v1/api-keys/{id}` revokes a key permanently.
...
you pay adds one dollar to your wallet, with no top-up fee.

You pay on Stripe's hosted checkout page, where you can use a saved payment method or save a
new reusable one. Your wallet updates only after AnyAPI receives the verified payment event.
Some payment methods settle later; the dashboard shows a delayed state until the top-up
lands.
