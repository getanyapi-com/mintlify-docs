...
charged only when a source charged AnyAPI for the lookup. When more than one source was
asked, each one that charged is billed at its own price, up to the API's published maximum;
a miss no source billed is free. Read `costUsd` for what the call actually cost, which is
