...
<AccordionGroup>
  ...
  <Accordion title="run_api — execute an API" icon="bolt">
    ...
    **Duplicate calls are not charged twice.** MCP clients re-issue a tool call on
    ...
    turn, a conversation branch is replayed. AnyAPI dedupes automatically, with no
    argument for you to pass. For about five minutes after a run, the same `sku_id`
    and the same `input` (compared by meaning, so key order and formatting do not
    matter) replays the original result instead of running again: `replayed` comes
    back `true`, `costUsd` restates the original charge, and nothing new is billed.
    Repeat the call while the original is still running and you get
    `duplicate_call_in_progress` (nothing charged) - wait a few seconds and repeat the
    same call to receive the result. Past that window, or with any change to the
