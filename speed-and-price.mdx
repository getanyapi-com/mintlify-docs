...
By default AnyAPI picks the cheapest source that can serve your request, and falls back to
the next if it fails. On many APIs the cheapest source is also much slower. Use
`preferLatencyUnderMs` when waiting costs you more than the price difference.

## The default: cheapest serves

Without the field, the cheapest source serves and you are charged its price.
...
**Your request is never refused for being slow.** The field changes the order sources are
tried in and removes none of them, so failover works as it does without it.

## It can cost more

When the cheapest source misses your target, a faster and dearer one serves, and you are
quoted and charged its price.

You can compare before you call, without a key. `GET /catalog` publishes each source's
`pricing` next to `health.latencyP50Ms`, the median routing reads:
...
With no preference, Badger serves at $0.0009 a call and a ~10.8 second
...
The field is accepted on every API and does nothing in these cases:
...
A cursor pins the walk to one source. See [Pagination](/pagination).
...
Compare `costUsd` on calls with and without the field to see the trade in your own numbers.
