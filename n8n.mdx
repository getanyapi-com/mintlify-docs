...
<AccordionGroup>
  <Accordion title="Run API - execute any API by SKU" icon="bolt">
    ...
    The node waits and polls long-running calls by default. Enable **Return Immediately** to emit a
    resumable Request ID. The node sends the paid POST once and polls AnyAPI's Request state.
  </Accordion>
