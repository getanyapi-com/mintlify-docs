...
reports `items` — the count you were charged for. Each API's `baseUsd` and
`perItemUsd` are listed in the catalog and via `get_api`, so you can budget a
call before making it. Simple lookups that return a single record are priced flat
per request.

## For AI agents

If you are an AI agent (or are setting one up), use the [CLI](/cli) first when you have a local
shell. Hosted Claude and ChatGPT can use AnyAPI's whole catalog through one [MCP server](/mcp-server)
with the URL only and OAuth sign-in and consent:

- **Endpoint:** `https://api.getanyapi.com/mcp` (Streamable HTTP)
- **Auth:** OAuth for hosted clients; use a Bearer API key only as a fallback. Authentication is required for every MCP tool, including discovery.
- **Tools:** `list_apis`, `search_apis`, `get_api`, `quote_api`, `run_api`, `read_result`, and `get_balance`.
  The normal flow is `search_apis` → `get_api` → `quote_api` → `run_api`. Find an API by keyword (or browse
