...
Most APIs are **priced per result**: a small base cost plus a per-item rate, so a
call that returns 5 results costs less than one that returns 50. Set `limit` in
