...
<Info>
  Only some catalog APIs are open for self-serve registration. The list, with the request,
  response, and published price for each one, is [APIs open to sellers](/sell/apis/index).
  To sell any other API, email [support@getanyapi.com](mailto:support@getanyapi.com).
</Info>
...
across 7 days from its first request. Nobody is billed for trial traffic and you are not
paid for it.

Every trial response is compared with what the customer was actually served:
...
If you ship a fix part-way through the trial, it is measured by the paced requests that
are still to come, so a bad first day is not permanent.
