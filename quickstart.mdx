...
<Steps>
  <Step title="Create an API key">
    ...
    New accounts start with about **100 free requests**, so you can make your first calls
    before topping up.
    ...
    <Tip>
      ...
      $0.05, creates no account, and self-expires in 7 days. When the budget runs out, calls
  ...
  <Step title="Read the response">
    ...
    <Note>
      ...
      can tell. Each API's output schema lists the exact reasons it can answer with. You are charged for it only when a
      source charged AnyAPI for the lookup. When AnyAPI asked more than one source, each one
      that charged is billed at its own price, up to the API's published maximum; a miss no
      source billed is free. Failed calls are never charged, and `costUsd` is always what
      this call actually cost you - `0` when nothing was charged for it. See [Not found vs error](/sdks#not-found-vs-error)
      for the full list of outcomes.
    </Note>
  </Step>
</Steps>
...
You don't need to know SKUs ahead of time. To browse by eye, every platform has a
catalog page listing its endpoints, schemas, and USD prices: start at
[the full catalog](https://getanyapi.com/catalog), or jump straight to a platform such as
...
USD balance you draw down per request. Pay on Stripe's secure hosted checkout page; enabled
methods fund the same USD wallet with no AnyAPI top-up fee. The balance changes only after
the verified payment settles.
