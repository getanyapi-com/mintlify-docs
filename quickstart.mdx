...
- An AnyAPI account — sign up at [app.anyapi.dev](https://app.anyapi.dev).
...
<Steps>
  <Step title="Create an API key">
    In the [dashboard](https://app.anyapi.dev), open **API Keys** and create a new
  ...
  <Step title="Make your first call">
    ...
    <CodeGroup>
    ```bash curl
    curl -s https://api.anyapi.dev/v1/run/tiktok.profile \
    ...
    res = requests.post(
        "https://api.anyapi.dev/v1/run/tiktok.profile",
    ...
    ```javascript node
    const res = await fetch("https://api.anyapi.dev/v1/run/tiktok.profile", {
      method: "POST",
      headers: {
...
curl -s "https://api.anyapi.dev/v1/apis?query=tiktok" -H "X-API-Key: YOUR_ANYAPI_KEY"

# Inspect one API's input and output schemas
curl -s https://api.anyapi.dev/v1/apis/tiktok.profile -H "X-API-Key: YOUR_ANYAPI_KEY"
```
...
```bash
curl -s https://api.anyapi.dev/v1/balance -H "X-API-Key: YOUR_ANYAPI_KEY"
```

Top up any time from the [dashboard](https://app.anyapi.dev) — your wallet is a
