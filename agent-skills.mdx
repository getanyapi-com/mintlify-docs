...
For a local shell, use the [CLI](/cli) first. Hosted Claude and ChatGPT should connect to the
remote MCP URL only and complete OAuth sign-in and consent. Use a Bearer API key only for a
...
- The **discover → inspect → quote → run** flow over MCP or REST.
