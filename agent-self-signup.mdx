...
If it has a local shell, it uses the CLI first. Hosted Claude and ChatGPT use the remote MCP URL
only and ask you to sign in and consent through OAuth when the trial needs to continue. A Bearer
API key is only the fallback for clients that cannot do OAuth.
