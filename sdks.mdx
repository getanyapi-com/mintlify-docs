...
A `found: false` answer is billed only when a source charged AnyAPI for the lookup. On an
API with more than one source, every source AnyAPI asked that charged for the call is
billed at its own price, and the total never exceeds the API's published maximum. A miss
the sources refused without billing is free, and `costUsd` / `cost_usd` on the result is
always what the call actually cost you.
