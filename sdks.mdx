...
`output.reason`. The reason is one word from a fixed list that is part of every API's output
schema, so it is typed in both SDKs and never the source's own wording:
...
A `found: false` answer is billed only when the source charged AnyAPI for the lookup. A
...
`found: false` branch of `Output<T>`; in Python it is `OutputNotFound.reason`. New words can
be added to the list; read it from the API's `outputSchema` in `/v1/apis/{slug}` or the
OpenAPI document rather than hardcoding it.
...
The SDKs transparently follow a `202 Accepted` response by polling the durable Request. The overall
default remains 300 seconds. If that deadline expires while work is still active, the SDK throws
`RequestPendingError` with the resumable Request ID instead of losing the operation.

Use `start` when you want the handle immediately. If the same idempotency key already completed,
`start` returns that terminal run result as `200` instead. Use `requests.get` or `requests.wait` to
resume an accepted Request:
